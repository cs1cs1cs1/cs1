...
val_remove: (eq a) => a -> list a -> list a
val_remove v nil = nil
val_remove v (h::t) = ite (eql v h)
                          (t)
                          (h::(val_remove v t))
...
-- Return true if p is true for some v in s, otherwise else false
-- set_exists: (p: a -> bool) -> (s: set a) -> bool
set_exists p (mkSet l) = ite (member true (map p l))
                                true
                                false
...
list_exists p l = ite (member true (map p l))
                      true
                      false
...
-- pairelt takes an elt h and a list b and returns lists of all pairs
